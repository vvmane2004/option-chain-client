...
import { OptionChainOpenInterest } from '../../../interfaces/option-chain.interface';

Chart.register(...registerables, zoomPlugin);

@Component({
  selector: 'app-chart-openinterest',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './chart-openinterest.component.html',
  styleUrl: './chart-openinterest.component.scss'
})
export class ChartOpeninterestComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  loadData(): void {
    ...
    this.optionChainService.getOpenInterest(this.symbol, this.expiration, this.side)
      .subscribe({
        next: (data) => {
          this.data = data;
          this.createChart();
        ...
        error: (error) => {
          ...
          this.error = 'Failed to load open interest data';
          this.loading = false;
          this.clearChart();
        }
      });
    ...
    const config: ChartConfiguration = {
      ...
      options: {
        ...
        plugins: {
          title: {
            display: true,
            text: `${this.symbol} - ${this.side?.toUpperCase()} Open Interest by Strike`,
            font: {
              size: 16,
              weight: 'bold'
            }
  ...
  private groupDataByStrikeAndDate(): { [strike: number]: { [date: string]: number } } {
    ...
    this.data.forEach(item => {
      if (item.imported_date && item.open_interest !== undefined && item.strike !== undefined) {
        ...
        grouped[strike][date] = item.open_interest;
      }
    });
