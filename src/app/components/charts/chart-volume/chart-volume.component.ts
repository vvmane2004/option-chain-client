...
import { OptionChainVolume } from '../../../interfaces/option-chain.interface';
...
@Component({
  ...
  styleUrl: './chart-volume.component.scss'
})
export class ChartVolumeComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  loadData(): void {
    ...
    this.optionChainService.getVolume(this.symbol, this.expiration, this.side)
      .subscribe({
        next: (data) => {
          this.data = data;
          this.createChart();
        ...
        error: (error) => {
          ...
          this.error = 'Failed to load volume data';
          this.loading = false;
          this.clearChart();
        }
      });
    ...
    const config: ChartConfiguration = {
      ...
      options: {
        ...
        plugins: {
          title: {
            display: true,
            text: `${this.symbol} - ${this.side?.toUpperCase()} Volume by Strike`,
            font: {
              size: 16,
              weight: 'bold'
            }
  ...
  private groupDataByStrikeAndDate(): { [strike: number]: { [date: string]: number } } {
    ...
    this.data.forEach(item => {
      if (item.imported_date && item.volume !== undefined && item.strike !== undefined) {
        ...
        grouped[strike][date] = item.volume;
      }
    });
  ...
  getVolumeRange(): string {
    ...
    const minVolume = volumes[0]!.toLocaleString();
    const maxVolume = volumes[volumes.length - 1]!.toLocaleString();

    return minVolume === maxVolume ? minVolume : `${minVolume} - ${maxVolume}`;
  }
